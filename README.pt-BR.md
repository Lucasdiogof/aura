...
**Praticar** *(aba)*
- Três opções: prática rápida, revisar erros, favoritos
...
- A prática rápida é a única opção cuja interface existe mas ainda não abre um destino
...
2. No SQL Editor do seu projeto Supabase, rode os arquivos em `supabase/`: primeiro os arquivos de schema (`*_schema.sql`, mais o `map_quiz_progress.sql`), depois os seeds de questões de cada matéria, depois os arquivos `difficulty_classify_*.sql`, e por fim os arquivos de dossiê da Atualidades. `supabase/health_check.sql` e `supabase/audit_questions_duplicates.sql` são scripts só de leitura que você pode rodar a qualquer momento pra auditar o catálogo e o banco de questões.
3. `flutter pub get`
